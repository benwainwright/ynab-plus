...
export const getOauthClientFactory = (
  ...
  provider: string
) => IOauthRedirectUrlGenerator & IOauthNewTokenRequester & IOAuthTokenRefresher) => {
  const ynabClientId = bootstrapper.configValue(`ynabClientId`, z.string());

  const ynabClientSecret = bootstrapper.configValue(`ynabClientSecret`, z.string());

  const ynabRedirectUri = bootstrapper.configValue(`ynabRedirectUri`, z.string());

  return (provider: string) => {
    switch (provider) {
      case "ynab":
        return new YnabOauth2Client(ynabClientId, ynabClientSecret, ynabRedirectUri, `ynab`);
    }
