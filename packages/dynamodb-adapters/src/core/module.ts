...
import { DynamodbSyncDetailsAdapter } from "@adapters";
...
export const dynamodbDataAdaptersModule: TypedContainerModule<IDataPorts & IInternalTypes> =
  typedApplicationModule<IDataPorts & IInternalTypes>(({ load, logger }) => {
    logger.debug(`Initialising dynamodb adapters module`, LOG_CONTEXT);

    load.bind("SyncDetailsRepository").to(DynamodbSyncDetailsAdapter);
